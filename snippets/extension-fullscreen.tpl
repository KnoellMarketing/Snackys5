...
{if !$isQuickview}
	...
	{else if $snackyConfig.fullscreenElement == 1}
		...
		{else}
			<div class="video-background flx-ac flx-jc alert"><h2 class="m0">No Slider found</h2></div>
		{/if}
	{else if $snackyConfig.fullscreenElement == 2}
		{if isset($oImageMap)}
			{include file="snippets/banner.tpl"}
		{else}
			<div class="video-background flx-ac flx-jc alert"><h2 class="m0">No Banner found</h2></div>
		{/if}
	{/if}
{/if}
{/block}